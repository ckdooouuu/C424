...
public class Ping implements Runnable {
    ...
    public interface PingListener {
        void onPing(int index, ProxyInfo proxyInfo);

        void onPingException(Exception e, int count, ProxyInfo proxyInfo);
    }

    public Ping(final ProxyInfo proxyInfo, final InetAddress dest, final PingListener listener) {
        mDest = dest;
...
//                    for (int i = 0; i < mCount; i++) {
                    ...
                    try {
                        ...
                        final int[] rc = {sendto(fd, byteBuffer)};
                        if (rc[0] >= 0) {
                            rc[0] = poll(structPollfds);
                            final long time = calcLatency(start, System.currentTimeMillis());
                            if (rc[0] >= 0) {
                                if (structPollfd.revents == POLLIN) {
                                    structPollfd.revents = 0;
                                    rc[0] = recvfrom(fd, buffer);
                                    if (rc[0] < 0) {
                                        Log.d(TAG, "recvfrom() return failure: " + rc[0]);
                                    }
                                    proxyInfo.setDelay(time);
                                    mListener.onPing(0, proxyInfo);
                                } else {
                                    proxyInfo.setDelay(1000);
                                    mListener.onPing(0, proxyInfo);
                                }
                            } else {
                                mListener.onPingException(new IOException("poll() failed"), 0, proxyInfo);
                            }
                        } else {
                            mListener.onPingException(new IOException("sendto() failed"), 0, proxyInfo);
                        }
                    } catch (ErrnoException e) {
                        mListener.onPingException(e, 0, proxyInfo);
                    }
                    sleep();
                } finally {
                    close(fd);
                ...
                mListener.onPingException(new IOException("Invalid FD " + fd.toString()), 0, proxyInfo);
            }
